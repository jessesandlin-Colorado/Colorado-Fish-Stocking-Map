...
(function(){
  ...
  function goldMedalBadge(w){
    return window.cofishGoldMedal?.(w)?'<span class="gold-medal-badge">Gold Medal Water</span>':'';
  }
  ...
  render=function(){
    ...
    visible.forEach(w=>{
      ...
      if(window.cofishGoldMedal?.(w))n.querySelector('.card-name').insertAdjacentHTML('afterend',goldMedalBadge(w));
    ...
    if(markerLayer){
      ...
      visible.filter(w=>Number.isFinite(Number(w.lat))&&Number.isFinite(Number(w.lng))).forEach(w=>{
        const icon=L.divIcon({className:'',html:`<div class="marker-pin ${recency(w.latest_report_date)} ${waterTypeClass(w)} ${window.cofishHasStreamflow?.(w)?'has-streamflow':''} ${window.cofishGoldMedal?.(w)?'gold-medal-water':''}">${flowGauge(w)}</div>`,iconSize:[24,24],iconAnchor:[12,12]});
