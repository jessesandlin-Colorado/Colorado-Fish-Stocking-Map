...
function cleanSidebarSpeciesNames(root=document){root.querySelectorAll('.card-species').forEach(el=>{const raw=el.textContent.trim();if(!raw||raw==='Species not exposed')return;const cleaned=raw.split(' · ').map(name=>speciesInfo(name).displayName).join(' · ');if(cleaned!==raw)el.textContent=cleaned})}
function cleanSpeciesSelect(){const select=document.getElementById('species');if(!select||select.options.length<2)return false;for(const option of select.options){if(option.value){const cleaned=speciesInfo(option.value).displayName;if(option.textContent!==cleaned)option.textContent=cleaned}}return true}
